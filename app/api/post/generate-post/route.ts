...
function buildSystemPrompt(channelType?: string, characterLimit?: number) {
  ...
  if (channelType) {
    system_prompt.push(
      `Write for ${channelType}. Match the platform's tome, style, and expected length, and relevant hashtags`,
    );
  }

  if (characterLimit) {
    system_prompt.push(`The maximum character limit is ${characterLimit}`);
  }
