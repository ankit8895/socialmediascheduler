...
const GenerateIdeasPopover = ({ onGenerated }: GenerateIdeasPopoverProps) => {
  ...
  const generateMutation = useMutation({
    ...
    onError: (error) => {
      toast("Failed to generate ideas. Please try again");
    },
  });

  const handleGenerate = () => {
    if (!businessType || !targetAudience) {
      toast("Please provide both business type and target audience");
      return;
    }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant={"outline"}
          className="gap-2 bg-linear-to-r from-[#b0ec9c33] to-[#d1bdff33]"
        >
...
export default GenerateIdeasPopover;
