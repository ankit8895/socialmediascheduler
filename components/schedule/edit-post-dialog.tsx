...
const EditPostDialog = ({ open, onOpenChange, post }: EditPostDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          "sm:w-full sm:min-w-175 gap-0 px-0 pb-0!",
        ...
        <div>
          ...
          <div className="w-full flex flex-1 h-full min-h-137.5 max-h-142.5">
            ...
            <div className="flex flex-1 flex-col min-w-0 w-75 pb-5">
              <section className="channel--composer relative flex flex-col px-8 mt-5 min-h-75 h-auto">
            ...
            {/* RIGHT SIDE PANEL */}
            {selectedRightTab && (
              <aside className="w-87.5 shrink-0 border-l border-border bg-muted/30 h-full flex flex-col">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (isDatePassed || isTimeNotAvailable) {
                        toast("Please select a valid time");
                        return;
                      }
                      handleUpdate();
                    }}
