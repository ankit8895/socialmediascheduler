...
export const publishScheduledPostsCron = inngest.createFunction(
  ...
  async ({ step, logger }) => {
    ...
    return {
      message: "send out posts for publishing",
...
export const publishScheduledPost = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    ...
    const shouldRefreshBeforePublish =
      refreshToken && tokenExpiresAt !== null && tokenExpiresAt <= Date.now();
    ...
    try {
      ...
      return { failed: true, provider: providerType };
    }
  },
);
...
async function publishToLinkedIn({
  ...
  const body: Record<string, unknown> = {
    ...
    distribution: {
      ...
      targetEntities: [],
      thridPartyDistributionChannels: [],
    },
